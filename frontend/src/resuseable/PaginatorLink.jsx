...
const PaginatorLink = ({ paginate, itemsPerPage, totalItems, currentPage }) => {
    const totalPages = Math.ceil(totalItems / itemsPerPage);

    const pageNumber = [];

    for (let i = 1; i <= totalPages; i++) {  // Changed from < to <=
        pageNumber.push(i);
    }
                    ...
                    {displayPageNumber.map((number, index) => (
                        <li
                            ...
                            className={`flex items-center justify-center w-[32px] h-[32px] rounded-[50%] mx-1 
                                ${currentPage === number ? "text-white bg-[#4CAF50]" : "text-[#4B5563] bg-transparent"}`}
                        >
                            {number ? (
                                <a
                                    onClick={() => paginate(number)}
                                    className="w-full h-full flex items-center justify-center
                                     text-[14px] leading-[17px] font-[400] cursor-pointer font-tekInter "
                                >
                                    {number}
                                </a>
                            ) : (
