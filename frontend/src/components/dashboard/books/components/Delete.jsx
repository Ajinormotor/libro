...
const Delete = ({setShowDelete, showUser}) => {
const{deleteBook} = useBookStore()

// const location = useLocation()  

  const handleDelete = async () => {
    const { message, success } = await deleteBook(showUser._id)
...
<div className=' flex gap-4 items-center  bg-[#F9FAFB] rounded-lg p-3'>
     ...
     <img src={showUser?.image} alt='' loading='lazy'
    className='w-[150px] h-[100px] rounded-[4px] ' />

    <div className='flex flex-col'>
        <h1 className='text-[#111827] text-base font-bold'>{showUser?.bookName}</h1>
        <p className='text-[#6B7280] text-sm'> {showUser?.authorName}</p>
        <div className='flex items-center gap-2'>
